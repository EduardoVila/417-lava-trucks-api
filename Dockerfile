...
ENV BUNDLE_WITHOUT=development:test BUNDLE_DEPLOYMENT=true BUNDLE_PATH=/usr/local/bundle
RUN bundle install
...
COPY . .
ENV RACK_ENV=production APP_ENV=production PORT=8000 BUNDLE_WITHOUT=development:test BUNDLE_DEPLOYMENT=true BUNDLE_PATH=/usr/local/bundle
USER app
EXPOSE 8000
